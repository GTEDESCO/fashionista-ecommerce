...
export const filterProductBySlug = (pathname, data, callback) => {
  ...
  return callback(data)
    .then((products) =>
      products.filter((product) => slugfy(product.name) === slug),
    )
    .then((product) => product.reduce((single) => single));
};

export const removeItemBySlug = (itemName, arrayOfItems) =>
  arrayOfItems.filter((item) => slugfy(item.name) !== slugfy(itemName));

export const groupItemsBySlug = (itemName, arrayOfItems) =>
  arrayOfItems.filter((item) => slugfy(item.name) === slugfy(itemName));

export const findIndexBySlug = (itemName, arrayOfItems) =>
  arrayOfItems.findIndex((item) => slugfy(item.name) === slugfy(itemName));

export const sortByItemName = (items) =>
  items.sort((a, b) => a.name.localeCompare(b.name));

export const searchByTerms = (searchTerm, items) =>
  items.filter((item) => {
